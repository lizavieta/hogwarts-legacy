...
public class Wizard implements MagicSource, Trader, MagicEffectRealization, Rage {
	...
	public Wizard(String name, MagicLevel level, int basicHP, int HP, int basicMP, int MP, int money,
		...
		if (basicHP < 0)
			throw new IllegalArgumentException("Illegal basicHP");
		if (HP < 0) //defaults to basicHP
			//this.HP = basicHP;
			throw new IllegalArgumentException("Illegal HP");
		if (basicMP < level.toMana())
			throw new IllegalArgumentException("Illegal basicMP");
		if (MP < 0) //defaults to basicMP
			//this.MP = basicMP;
			throw new IllegalArgumentException("Illegal MP");
		if (money < 0)
			throw new IllegalArgumentException("Illegal money");
		if (knownSpells == null)
			throw new IllegalArgumentException("Illegal knowSpells");
		if (protectedFrom == null)
			throw new IllegalArgumentException("Illegal protectedFrom");
		if (carryingCapacity < 0)
			throw new IllegalArgumentException("Illegal carryingCapacity");
		if (inventory == null)
			throw new IllegalArgumentException("Illegal inventory");
		this.name = name;
		this.level = level;
	...
	public Wizard(String name, MagicLevel level, int basicHP, int basicMP, int money,
				  Set<Spell> knownSpells, Set<AttackingSpell> protectedFrom, int carryingCapacity,
				  Set<Tradeable> inventory) {
		this(name, level, basicHP, basicHP, basicMP, basicMP, money, knownSpells, protectedFrom,
	...
	public boolean learn(Spell s) {
		if (s == null)
			throw new IllegalArgumentException("Illegal Spell");
		if (isDead())
			return false;
	...
	public boolean castSpell(Spell s, MagicEffectRealization target) {
		if (s == null || target == null) {
			throw new IllegalArgumentException("Illegal spell or target");
		}
	...
	public boolean castRandomSpell(MagicEffectRealization target) {
		...
		Spell[] arraySpells = knownSpells.toArray(new Spell[0]);
		Random rndm = new Random();
		int rndmNumber = rndm.nextInt(knownSpells.size());
		return castSpell(arraySpells[rndmNumber], target);
	}
	...
	public boolean useItem(Tradeable item, MagicEffectRealization target) {
		if (item == null || target == null) {
			throw new IllegalArgumentException("Illegal item or target");
		}
	...
	public boolean useRandomItem(MagicEffectRealization target) {
		...
		Tradeable[] arrayInventory = inventory.toArray(new Tradeable[0]);
		Random rndm = new Random();
		int rndmNumber = rndm.nextInt(inventory.size());
		return useItem(arrayInventory[rndmNumber], target);
	}
	...
	public boolean sellRandomItem(Trader target) {
		...
		Random rndm = new Random();
		int rndmNumber = rndm.nextInt(inventory.size());
		return sellItem(arrayInventory[rndmNumber], target);
	}
	...
	public String toString() {
		String levelAsterisks = level.toString();
		String currency = (money == 1) ? "Knut" : "Knuts";
		String knownSpellsString = knownSpells.toString();
		String inventoryString = inventory.toString();
		return "[" + name + "(" + levelAsterisks + "): " +
				HP + "/" + basicHP + " " + MP + "/" + basicMP + "; " +
				money + " " + currency + "; knows " + knownSpellsString +
	...
	public boolean provideMana(MagicLevel levelNeeded, int manaAmount) {
		...
		if (isDead() || level.compareTo(levelNeeded) < 0 || MP < manaAmount)
			return false;
		MP -= manaAmount;
		return true;
	}
	...
	public boolean addToInventory(Tradeable item) {
		if (item == null)
			throw new IllegalArgumentException("Item is null");
		if (possesses(item) || !hasCapacity(item.getWeight())) {
			return false;
		}
		inventory.add(item);
		return true;
	...
	public boolean steal(Trader thief) {
		...
		Random rndm = new Random();
		int rndmNumber = rndm.nextInt(inventory.size());
		Tradeable randomItem = arrayInventory[rndmNumber];
	...
	public boolean loot(Trader looter) {
		...
		if (isLootable() && !inventory.isEmpty()) {
        ...
        return (removedCheck && addedCheck);
	}
	...
	public void takeDamage(int amount) {
		if (amount < 0) {
			throw new IllegalArgumentException("Negative amount of damage");
		}
		if (amount < HP)
			HP -= amount;
		else
	...
	public void takeDamagePercent(int percentage) {
		...
		int damageAmount = (int) ((percentage/100.00)* basicHP);
	...
	public void weakenMagicPercent(int percentage) {
		...
		int damageAmount = (int) ((percentage/100.00)* basicMP);
		weakenMagic(damageAmount);
	}
	...
	public void heal(int amount) {
		if (amount < 0) {
			throw new IllegalArgumentException("Negative amount of healing");
		}
		HP += amount;
	}
	...
	public void healPercent(int percentage) {
		if (percentage < 0 || percentage > 100) {
			throw new IllegalArgumentException("Invalid percentage of healing: less than 0 or more than 100");
		}
		int healAmount = (int) Math.max(1, (percentage / 100.0) * basicHP);
		heal(healAmount);
	}
	...
	public void enforceMagic(int amount) {
		if (amount < 0) {
			throw new IllegalArgumentException("Negative amount to increase mana");
		}
		MP += amount;
	}
	...
	public void enforceMagicPercent(int percentage) {
		if (percentage < 0 || percentage > 100) {
			throw new IllegalArgumentException("Invalid percentage to increase mana: less than 0 or more than 100");
		}
		int enforceAmount = (int) ((percentage/100.00)* basicMP);
		enforceMagic(enforceAmount);
	}
	...
	public boolean isProtected(Spell s) {
		...
		if (!(s.getClass() == AttackingSpell.class))
			return false;
...
///////////////////////////// TASKS /////////////////////////////////
	...
	public MagicItem[] getBestNItems(int n) {
		if (inventory == null || inventory.isEmpty() || n < 1)
			throw new IllegalArgumentException();
		return inventory
				.stream()
				.map(item -> (MagicItem) item)
				.limit(n)
				.sorted(new SortMagicItem())
				.toArray(MagicItem[]::new);
	}
	...
	public static TreeMap<MagicItem, TreeSet<Wizard>> getWizardsPerItem(Collection<Wizard> wizards) {
		TreeMap<MagicItem, TreeSet<Wizard>> result = new TreeMap<>(new SortTreeMap());
		for (Wizard wizard: wizards) {
			List<Tradeable> inventory = new ArrayList<>(wizard.getInventory());
			MagicItem[] keys = new MagicItem[inventory.size()];
			int i = 0;
			while (i < wizard.getInventory().size()) {
				keys[i] = (MagicItem) inventory.get(i);
				i++;
			}
			//MagicItem[] keys = wizard.getInventory().stream().map(item -> (MagicItem) item).toArray(MagicItem[]::new);
			for (MagicItem key: keys) {
                //Set<Wizard> values = result.computeIfAbsent(key, k -> new TreeSet<>(new SortTreeSet()));
				if (!result.containsKey(key)) {
					TreeSet<Wizard> values= new TreeSet<>(new SortTreeSet());
					result.put(key, values);
					values.add(wizard);
				}
				else
					result.get(key).add(wizard);
			}
		}
		return result;
	}
...
//Task 3 from test 2
class SortMagicItem implements Comparator<MagicItem> {
	public int compare(MagicItem lhs, MagicItem rhs) {
		return (rhs.getUsages() - lhs.getUsages());
	}
}

//Task 4 from test 2
class SortTreeMap implements Comparator<MagicItem> {
	public int compare(MagicItem lhs, MagicItem rhs) {
		return rhs.getPrice() - lhs.getPrice();
	}
}

class SortTreeSet implements Comparator<Wizard> {
	public int compare(Wizard lhs, Wizard rhs) {
