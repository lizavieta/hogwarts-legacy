...
public class Concoction extends Potion {
	...
	public Concoction(String name,	int usages, int price, int weight, int health, int mana, List<Spell> spells) {
		...
		if (spells == null) {
			throw new IllegalArgumentException("List of spells is null");
		}
		if (health == 0 && mana == 0 && spells.isEmpty()) {
			throw new IllegalArgumentException("Both health and mana cannot be 0");
		}
		this.health = health;
		this.mana = mana;
	...
	public String additionalOutputString() {
		...
		if (health != 0) {
			result.append("; ").append(health > 0 ? "+" : "-").append(Math.abs(health)).append(" HP");
		}
		if (mana != 0) {
			result.append("; ").append(mana > 0 ? "+" : "-").append(Math.abs(mana)).append(" MP");
		}
		if (!spells.isEmpty()) {
			result.append("; cast [");
			for (int i = 0; i < spells.size(); i++) {
				...
				if (i == 0) {
					...
					result.append(", ").append(spell.toString());
				}
			}
			result.append("]");
		}
